...
description: "小红书视频数据多维深度分析与复盘工具。当用户上传小红书数据文件（包含播放数据明细表.xlsx、互动数据明细表.xlsx、观看趋势.json等）或包含流量来源、观众画像的数据截图时，必须触发此 Skill。本 Skill 将通过多模态数据提取、指标自动计算、导出无样式 Excel 数据以及生成交互式 HTML Dashboard 完成闭环分析，甚至在用户只说「帮我整理下数据」或「看看这个视频表现」时，也应当优先使用本 Skill 进行分析服务。"
---
